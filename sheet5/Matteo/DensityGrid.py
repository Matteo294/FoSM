...
class DensityGrid:
    ...
    def add_particle(self, r):
        ...
        self.W1[k,l] += m*epsx * epsy
        self.W1[k,l+1] += m*epsx * (1-self.top_hat(epsy))*(1-epsy)
        self.W1[k,l-1] += m*epsx * self.top_hat(epsy)*(1-epsy)
        self.W1[k+1,l] += m*(1-self.top_hat(epsx))*(1-epsx) * epsy
        self.W1[k+1,l+1] += m*(1-self.top_hat(epsx))*(1-epsx) * (1-self.top_hat(epsy))*(1-epsy)
        self.W1[k+1,l-1] += m*(1-self.top_hat(epsx))*(1-epsx) * self.top_hat(epsy)*(1-epsy)
        self.W1[k-1,l] += m*self.top_hat(epsx)*(1-epsx) * epsy
        self.W1[k-1,l+1] += m*self.top_hat(epsx)*(1-epsx) * (1-self.top_hat(epsy))*(1-epsy)
        self.W1[k-1,l-1] += m*self.top_hat(epsx)*(1-epsx) * self.top_hat(epsy)*(1-epsy)
    ...
    # pass the position and the index of the closest cell
    def w2_func(self, x):
        if abs(x) > 3/2:
            return 0
