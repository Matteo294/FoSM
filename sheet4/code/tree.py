...
class TreeClass:
            ...
            for iz in range(0,2):
                for iy in range(0,2):
                    for ix in range(0,2):
                        ...
                        # ....TO BE FILLED IN....
                        mass += m
                        cenm = [cenm[i] + m*cm[i] for i in range(3)]
            cenm = [cenm[i]/mass for i in range(3)]
    ...
    def gforce(self,nodeid,pid,anglemax):
        ...
        #
        cm_mod2 = np.linalg.norm([cm[i]-x[i] for i in range(3)])**2
        if angle < anglemax or self.nodelist[nodeid].leaf == 1:
            ...
            #
            # ....TO BE FILLED IN....
            force = [-m*(x[i]-cm[i])/(cm_mod2 + 0.001**2)**(3/2) for i in range(3)]
