...
def get_acceleration(x,v,m,rho,P,a,pars):
    ...
    #TODO
    #EKIN = 0.5 * mp * numpy.linalg.norm(v)
    ...
    for i in range(0,pars[i_N]):
        ...
        #TODO:
        EINT += mp * P[i] / rho[i] / (pars[i_gamma]-1)
        EKIN += 0.5*mp*(v[0][i]**2 + v[1][i]**2 + v[2][i]**2)
...
x[0,:int(N/2)] = data['x'][0]
x[1,:int(N/2)] = data['x'][1]
x[2,:int(N/2)] = data['x'][2]
v[0,:int(N/2)] = data['v'][0]
v[1,:int(N/2)] = data['v'][1]
v[2,:int(N/2)] = data['v'][2]

x[0,int(N/2):] = data['x'][0] + 3.0
x[1,int(N/2):] = data['x'][1] + 1.5
x[2,int(N/2):] = data['x'][2]
v[0,int(N/2):] = data['v'][0] - 0.1
v[1,int(N/2):] = data['v'][1]
v[2,int(N/2):] = data['v'][2]
...
while(t<tmax):
    ...
    if(idx < 10):
        plt.savefig("twoplanets_video/sph_00" + str(idx) + ".png")
    elif(idx < 100 and idx >= 10):
        plt.savefig("twoplanets_video/sph_0" + str(idx) + ".png")
    else:
        plt.savefig("twoplanets_video/sph_" + str(idx) + ".png")

    idx += 1 
    t+=dt #update time
    dt,newE=leapfrog(x,v,m,rho,P,a,dt,pars) #leapfrog step and next time step estimate
    E.append(newE)
plt.close()
plt.figure()
times = numpy.linspace(0, tmax, len(E))

plt.subplot(2,1,1)
plt.plot(times, E, lw=1.8, color='black')
plt.ylabel("E", fontsize=12)
ax = plt.gca()
ax.tick_params(direction='in')

plt.subplot(2,1,2)
plt.plot(times, [abs((EE-E0)/E0) for EE in E], color='black', lw=1.8)
plt.xlabel("t", fontsize=12)
plt.ylabel(r"$\Delta E = \left|\frac{E-E_0}{E_0}\right|$", fontsize=12)
ax = plt.gca()
ax.tick_params(direction='in')
plt.show()
